...
def configure_opik(opik_mode: str = "hosted"):
    ...
    os.environ["OPIK_PROJECT_NAME"] = "opik-mcp-chatbot"
...
class MCPChatbot:
    ...
    @staticmethod
    def load_config(config_path: str = "config.json") -> tuple[List[ServerConfig], str, Dict[str, Any]]:
    ...
    async def _execute_tool_call(self, tool_call) -> str:
        ...
        try:
            ...
            if hasattr(result, "content") and result.content is not None:
                try:
                    ...
                    if self.debug:
                        ...
                        print(f"🔍 Content data keys: {content_data.keys() if isinstance(content_data, dict) else 'Not a dict'}")
                        ...
                        if hasattr(first_item, 'text'):
                            try:
                                ...
                                if (isinstance(text_data, dict) and 
                                    text_data.get("type") == "image_result" and
                                    "image_base64" in text_data):
                                    # Handle image result specially
                                    if self.debug:
                                        print(f"🖼️  Detected image result from {actual_tool_name}")
                                    return self._handle_image_result(text_data, actual_tool_name)
                    ...
                    elif (isinstance(content_data, dict) and 
            ...
            if len(content_str) > max_result_size:
                if self.debug:
                    print(f"⚠️  Large result detected: {len(content_str):,} characters (limit: {max_result_size:,})")
                return f"⚠️ **Large result from {actual_tool_name}**\n\n" \
                       f"📊 **Result Info:**\n" \
    ...
    def _handle_image_result(self, image_data: Dict[str, Any], tool_name: str) -> str:
        ...
        try:
            ...
            if self.debug:
                ...
                print(f"📐 Image dimensions: {image.size[0]}x{image.size[1]} pixels (resized to 400x300)")
            ...
            try:
                ...
                if shutil.which("kitty"):
                    ...
                    subprocess.run(["kitty", "+kitten", "icat", display_path], check=False)
            ...
            if image_displayed:
                return f"🖼️ **Image created by {tool_name}**\n\n" \
                       f"📊 **Image Details:**\n" \
            ...
            try:
                ...
                if server_name is None:
                    if not self.sessions:
                        ...
                        self.console.print("Use `!list_available_tools()` to see connection status.")
                        return None
                    ...
                    self.console.print("[bold blue]# Available MCP Servers and Tools[/bold blue]")
                    self.console.print()
                    
                    for server_data in data:
                        if server_data['error']:
                            self.console.print(f"[bold cyan]## 📡 Server: {server_data['name']}[/bold cyan]")
                            self.console.print()
                            self.console.print(f"[italic red]*Error getting tools: {server_data['error']}*[/italic red]")
                            self.console.print()
                        else:
                            self.console.print(f"[bold cyan]## 📡 Server: {server_data['name']}[/bold cyan]")
                            self.console.print()
                            ...
                            if server_data['tools']:
                                for tool in server_data['tools']:
                                    self.console.print(f"[green]-[/green] [bold]{tool['name']}[/bold]: {tool['description']}")
                            else:
                ...
                if server_name not in self.sessions:
                    ...
                    self.console.print(f"Server '{server_name}' not found. Available servers: {', '.join(available_servers)}")
                    return None
                ...
                # Now print the formatted output
                self.console.print(f"[bold blue]# Tools for Server: {server_name}[/bold blue]")
                self.console.print()
                        ...
                        if hasattr(target_tool, 'inputSchema') and target_tool.inputSchema:
                            ...
                            if 'properties' in target_tool.inputSchema:
                                for param_name, param_info in target_tool.inputSchema['properties'].items():
                                    ...
                                    required = param_name in target_tool.inputSchema.get('required', [])
        ...
        for round_num in range(self.max_rounds):
            try:
                ...
                if not tool_calls:
                    ...
                    if self.debug:
                        print(f"✅ LLM returned text response: {len(text_reply)} characters")
            ...
            for tc in tool_calls:
                ...
                with self.console.status(f"[bold blue]Executing {tc.function.name}...[/bold blue]", spinner="dots"):
            ...
            if self.debug:
                print(f"📝 Adding {len(executed_tool_msgs)} tool results to conversation history")
            ...
            if executed_tool_msgs and self.debug:
                ...
                print(f"🔍 Last tool result preview: {str(last_tool_result.get('content', ''))[:200]}...")
                print(f"🔍 Last tool result length: {len(str(last_tool_result.get('content', '')))}")
    ...
    async def run(self):
        ...
        try:
            ...
            self.console.print("[dim]Type '/debug on' or '/debug off' to toggle debug output.[/dim]")
            self.console.print("[dim]Type '/show tools' to list all available tools.[/dim]")
            self.console.print("[dim]Type '/show tools SERVER' to list tools for a specific server.[/dim]")
            self.console.print("[dim]Type '/run SERVER.TOOL [args]' to execute a tool.[/dim]")
            ...
            while True:
                ...
                if q in {""}:
                    ...
                    if len(parts) == 2:  # /show tools
                        ...
                        self.console.print("[yellow]Usage: /show tools [server_name][/yellow]")
                    continue
    ...
    async def _handle_run_tool(self, tool_command: str):
        ...
        try:
            ...
            # Parse tool_identifier to extract server and tool names
            if '.' not in tool_identifier:
                self.console.print(f"[red]Error: Tool identifier must be in SERVER.TOOL format, got: {tool_identifier}[/red]")
                return
            ...
            if server_name not in self.sessions:
                available_servers = list(self.sessions.keys())
                self.console.print(f"[red]Error: Server '{server_name}' not found. Available servers: {', '.join(available_servers)}[/red]")
                return
            ...
            session = self.sessions[server_name]
            try:
                self.console.print(f"[blue]🔧 Calling tool: {tool_name} with args: {kwargs}[/blue]")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m opik_mcp_chatbot config.json         # Use specific config with local Opik
  python -m comet_mcp_chatbot --opik hosted      # Use hosted Opik instance
  python -m comet_mcp_chatbot --opik disabled    # Disable Opik tracing
  python -m comet_mcp_chatbot --init             # Create default config.json
        """
    )
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug output during processing"
    )
...
"""
    ...
    bot = MCPChatbot(
        args.config_path, system_prompt=system_prompt, debug=args.debug
    )
    await bot.run()
