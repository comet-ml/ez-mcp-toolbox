...
class MCPEvaluator:
    ...
    def configure_opik(self):
        ...
        os.environ["OPIK_PROJECT_NAME"] = "ez-mcp-eval"

        # Check if ~/.opik.config file exists
        opik_config_path = os.path.expanduser("~/.opik.config")
        if os.path.exists(opik_config_path):
            self.console.print("✅ Found existing ~/.opik.config file, skipping opik.configure()")
            return

        try:
            import opik
            if self.config.opik_mode == "local":
                opik.configure(use_local=True)
            elif self.config.opik_mode == "hosted":
                opik.configure(use_local=False)
            else:
                self.console.print(f"Warning: Unknown Opik mode '{self.config.opik_mode}', using hosted mode")
                opik.configure(use_local=False)

            self.console.print("✅ Opik configured for evaluation")
        except Exception as e:
            self.console.print(f"Warning: Opik configuration failed: {e}")
            self.console.print("Continuing without Opik tracing...")
    ...
    def setup_client_and_dataset(self):
        ...
        try:
            ...
            self.console.print(f"   - Items count: {len(self.dataset) if hasattr(self.dataset, '__len__') else 'Unknown'}")
            
        except Exception as e:
            self.console.print(f"❌ Failed to load dataset '{self.config.dataset}': {e}")
            raise
    ...
    def resolve_prompt(self, prompt_value):
        ...
        try:
            ...
            self.console.print(f"✅ Found prompt in Opik: {prompt_content[:100]}{'...' if len(prompt_content) > 100 else ''}")
            return prompt_content
            
        except Exception as e:
            # If not found or any error, use the prompt value directly
            self.console.print(f"⚠️  Prompt '{prompt_value}' not found in Opik ({e}), using as direct prompt")
            return prompt_value
    ...
    def create_evaluation_task(self):
        ...
        def evaluation_task(dataset_item):
            ...
            try:
                ...
                try:
                    # Use litellm to call the LLM with Opik tracing
    ...
    def _call_llm_with_tracing(self, model: str, messages: List[Dict[str, Any]], **kwargs):
        """Call LLM with proper Opik span management."""
        try:
            if self.config.debug:
                self.console.print(f"🤖 Calling LLM: {model}")
                self.console.print(f"📝 Messages count: {len(messages)}")
                if kwargs:
                    self.console.print(f"⚙️  Model kwargs: {kwargs}")
            ...
            if self.config.debug:
                self.console.print(f"📊 LLM response type: {type(resp)}")
            if resp is None:
                if self.config.debug:
                    self.console.print("❌ LLM returned None response")
                raise ValueError("LLM returned None response")

            if not hasattr(resp, "choices"):
                if self.config.debug:
                    self.console.print(f"❌ LLM response missing 'choices' attribute: {resp}")
                raise ValueError(f"LLM response missing 'choices' attribute: {resp}")

            if self.config.debug:
                self.console.print(f"✅ LLM response has {len(resp.choices)} choices")
            
            return resp.choices[0].message.content

        except Exception as e:
            if self.config.debug:
                self.console.print(f"❌ LLM call failed: {e}")
                self.console.print(f"❌ Exception type: {type(e)}")
            raise
    ...
    def get_metrics(self):
        ...
        for metric_name in metric_names:
            ...
            try:
                ...
                self.console.print(f"❌ Unknown metric '{metric_name}'. Available metrics:")
    ...
    def run_evaluation(self):
        ...
        try:
            ...
            resolved_prompt = self.resolve_prompt(self.config.prompt)
            prompt_display = resolved_prompt[:100] + "..." if len(resolved_prompt) > 100 else resolved_prompt
...
def parse_field_mapping(mapping_str):
    ...
    if "=" not in mapping_str:
        raise ValueError(f"Field mapping must be in format REFERENCE=QUESTION, got: {mapping_str}")
...
def parse_output_mapping(mapping_str):
    ...
    if "=" not in mapping_str:
        raise ValueError(f"Output mapping must be in format reference=DATASET_FIELD, got: {mapping_str}")
...
def list_available_metrics():
    ...
    available_metrics = [name for name in dir(metrics) if not name.startswith('_') and callable(getattr(metrics, name))]
